...
const SearchPageCombineKeynoSynonyms = () => {
  ...
  const debouncedSearch = useCallback(
    debounce(async (searchQuery) => {
      setIsLoading(true);
      try {
        const response = await searchDataKeynoSynonyms(searchQuery);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      setIsLoading(true);
      try {
        const response = await searchDataKeynoSynonyms('');
...
export default SearchPageCombineKeynoSynonyms;