...
const SearchPageCloud = () => {
  ...
  const debouncedSearch = useCallback(
    debounce(async (searchQuery) => {
      setIsLoading(true);
      try {
        const response = await searchCloud(searchQuery);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      setIsLoading(true);
      try {
        const response = await searchCloud('');
...
export default SearchPageCloud;